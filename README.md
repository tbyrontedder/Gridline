...
**Fill from Example (Data):** Select a source column or range without its header, enter a household-name example (or use the existing example immediately to the right), and review the preview. This focused version produces `SURNAME, Given names`: it normalizes `/`, `&`, and `and`, preserves hyphenated surnames, uses the first person's surname when supplied, otherwise the last word of the household, and keeps only the first two names in comma-separated lists. Existing destination entries are preserved; blank/unrecognized inputs are skipped. It writes fixed text locally with single-step Undo/Redo. It does not provide general Excel Flash Fill inference; review compound surnames and unusual name structures carefully.

XLSX exports use native raw DEFLATE compression when available, with an uncompressed ZIP fallback for older browsers. No compression dependency is required. The embedding API is now asynchronous: use `await Gridline.exportXLSX(workbook)` to obtain the XLSX bytes.
