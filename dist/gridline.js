...
function numberFormatCode(style = {}) {
...
function formattedInput(raw, style = {}) {
...
function duplicateValues(workbook, sheet, range) {
 ...
 */
const { duplicateValues, MAX_ROWS, MAX_COLS, colName, FormulaError, formatValue } = __modules["engine"];
...
class GridRenderer {
  ...
  syncLayout() {
    const sheet = this.workbook.activeSheet, populated = sheet.populatedRange({r1:0,c1:0,r2:MAX_ROWS-1,c2:MAX_COLS-1}); this.populatedRows = populated.r2 + 1; this.populatedCols = populated.c2 + 1; this.cols = new AxisLayout(MAX_COLS, 106, sheet.colWidths, sheet.hiddenCols); this.rows = new AxisLayout(MAX_ROWS, 27, sheet.rowHeights, sheet.hiddenRows); this.requestFrame();
  }
  setZoom(zoom) {
...
const { Workbook, Sheet, keyOf, address, parseAddress, parseRange, shiftFormula, FormulaError, rawValue, numberFormatCode, numberFormatStyle } = __modules["engine"];
...
async function importXLSX(buffer, title = 'Imported workbook') {
  ...
  for (const node of elements(workbookDoc, 'sheet')) {
    ...
    const tabRGB = firstElement(doc, 'tabColor')?.getAttribute('rgb');
    sheet.color = /^(?:[0-9a-f]{2})?[0-9a-f]{6}$/i.test(tabRGB || '') ? '#' + tabRGB.slice(-6).toLowerCase() : null;
...
class GridlineApp {
  ...
  observeWorkbook() {
    this.unsubscribe?.(); this.unsubscribe = this.workbook.onChange(() => {
      ...
      $('#save-status').textContent = this.autosave ? 'Saving in this browser…' : 'Autosave is off';
  ...
  persist() {
    ...
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(this.workbook.toJSON())); $('#save-status').textContent = 'Saved in this browser'; return true; }
  ...
  bindEvents() {
    ...
    this.host.addEventListener('dblclick', e => { if (e.target.closest('.chart-card,.scroll-thumb,.cell-editor')) return; const p = this.localPoint(e), hit = this.renderer.hitTest(p.x, p.y), end = this.renderer.cellRect(this.selection.r2,this.selection.c2,false); if (!hit.colHeader && !hit.rowHeader && Math.abs(p.x-end.x-end.w)<7 && Math.abs(p.y-end.y-end.h)<7) { this.errorBoundary(() => this.autoFillDown()); return; } if (hit.colHeader) this.autoFit(this.selection.r1 === 0 && this.selection.r2 === MAX_ROWS - 1 && hit.c >= this.selection.c1 && hit.c <= this.selection.c2 ? null : hit.c); else if (!hit.rowHeader) this.startEdit(); });
    ...
    $('#sheet-tabs').addEventListener('contextmenu', e => { const t = e.target.closest('[data-sheet]'); if (t) { e.preventDefault(); this.switchSheet(t.dataset.sheet); this.contextMenu(e.clientX, e.clientY, [['rename-sheet','Rename…'],['tab-color','Tab Color…'],['duplicate-sheet','Duplicate'],['add-sheet','Insert worksheet'],null,['delete-sheet','Delete worksheet']]); } });
  ...
  onPointerDown(e) {
    ...
    if (hit.colHeader && !hit.rowHeader && Math.abs(p.x - colRect.x - colRect.w) < 6 && this.editable()) this.drag = { mode: 'resize-col', index: hit.c, start: p.x, size: this.renderer.cols.size(hit.c) };
      ...
      if (!hit.colHeader && !hit.rowHeader && Math.abs(p.x - end.x - end.w) < 7 && Math.abs(p.y - end.y - end.h) < 7 && this.editable()) this.drag = { mode: 'fill', source: { ...this.selection }, start: p };
      else {
  ...
  onPointerMove(e) {
    ...
    if (drag.mode === 'fill' && !drag.target && Math.hypot(p.x - drag.start.x, p.y - drag.start.y) < 5) return;
  ...
  renderRibbon() {
    ...
    let html = '';
    if (this.tab === 'Home') {
      html += group('Clipboard', `<div class="paste-tools">${tool('paste','Paste','paste',true)}<button data-action="paste-menu" aria-label="Paste options" title="Paste options" style="width:100%;height:16px">⌄</button></div>` + stack(tool('cut','Cut','cut'),tool('copy','Copy','copy')) + stack(tool('format-painter','Format painter','paint')));
  ...
  renderTabs() {
    $('#sheet-tabs').innerHTML = this.workbook.sheets.map(s => `<button class="sheet-tab ${s.id === this.sheet.id ? 'selected' : ''}" data-sheet="${escapeHTML(s.id)}" title="Double-click to rename · Right-click for options"><span class="sheet-tab-dot" style="background:${/^#[0-9a-f]{6}$/i.test(s.color) ? s.color : 'transparent'}"></span>${escapeHTML(s.name)}</button>`).join('');
  }
  switchSheet(id) {
    if (id === this.sheet.id) return; if (['find','analyze'].includes(this.panelType)) this.closePanel(); this.commitEdit(false); this.commitFormula(); this.sheetViews.set(this.sheet.id, { selection: { ...this.selection }, active: { ...this.active }, scrollX: this.renderer.scrollX, scrollY: this.renderer.scrollY });
  ...
  showColorPicker(property) {
    if (!this.editable()) return;
    const sheet = this.sheet, q = {...this.selection}, current = property === 'tab' ? sheet.color : sheet.style(this.active.r,this.active.c)[property];
    ...
    this.openDialog(property === 'tab' ? 'Tab Color' : property === 'fill' ? 'Fill Color' : 'Text Color', `<div style="display:grid;grid-template-columns:repeat(6,1fr);gap:8px;margin-bottom:18px">${colors.map(color=>`<button type="button" data-swatch="${color}" aria-label="Color ${color}" title="${color}" style="height:32px;background:${color};border:1px solid var(--line)"></button>`).join('')}</div><label class="field-label" for="color-picker">Custom color</label><input id="color-picker" type="color" value="${initial}" style="width:100%;height:40px"><label class="field-label" for="color-hex">Hex color</label><input id="color-hex" class="dialog-input" value="${initial}" pattern="#[0-9a-fA-F]{6}" required maxlength="7"><div class="dialog-actions">${property === 'tab' ? '<button id="color-clear" class="secondary-btn">No Color</button>' : ''}<button class="secondary-btn" data-action="close-dialog">Cancel</button><button id="color-apply" class="primary-btn">Apply</button></div>`, 360);
    if (property === 'tab') $('#color-clear').onclick = () => this.errorBoundary(() => { this.workbook.mutate('Clear tab color', () => { sheet.color = null; }); this.closeDialog(); });
    ...
    $('#color-apply').onclick = () => this.errorBoundary(() => { if (!hex.reportValidity()) return; if (property === 'tab') this.workbook.mutate('Change tab color', () => { sheet.color = hex.value.toLowerCase(); }); else this.workbook.applyStyle(sheet,q,{[property]:hex.value.toLowerCase()}); this.closeDialog(); });
  }
  showFormatCells() {
    if (!this.editable()) return;
  ...
  autoFillDown() {
    if (!this.editable()) return;
    const source = {...this.selection}, sheet = this.sheet;
    if (source.r1 !== source.r2 || source.c1 !== source.c2 || !sheet.raw(source.r1,source.c1)) return;
    const neighbors = [source.c1-1,source.c2+1].filter(c => c >= 0 && c < MAX_COLS);
    let last = source.r2;
    while (last < MAX_ROWS-1 && neighbors.some(c => sheet.raw(last+1,c) !== '')) last++;
    if (last === source.r2) return;
    const target = {...source,r2:last};
    if (sheet.merges.some(m => m.r1<=last && m.r2>=source.r1 && m.c1<=source.c1 && m.c2>=source.c1)) throw new Error('Unmerge these cells before filling down.');
    this.workbook.fill(sheet,source,target); this.select(target,this.active);
  }
  autoFit(column = null) {
  ...
  async run(action, element = null) {
    ...
    switch (action) {
      ...
      case 'paste-menu': return this.menuAt(element,[['paste','Paste'],['paste-values','Paste as Values'],['paste-formula','Paste as Formula']]);
      case 'paste': case 'paste-values': case 'paste-formula': return this.paste(action);
      ...
      case 'tab-color': return this.showColorPicker('tab');
  ...
  clipboardPayload(cut = false) {
    const q = this.selection.r2-this.selection.r1===MAX_ROWS-1 || this.selection.c2-this.selection.c1===MAX_COLS-1 ? this.sheet.populatedRange(this.selection) : {...this.selection}, rows = [], cells = [], values = [];
    for (const { r,c } of cellsIn(q)) {
      if (!rows[r-q.r1]) { rows[r-q.r1] = []; cells[r-q.r1] = []; values[r-q.r1] = []; }
      const value = this.workbook.value(this.sheet,r,c); values[r-q.r1][c-q.c1] = value instanceof FormulaError ? value.code : value;
    ...
    const text = serializeDelimited(rows,'\t'); this.clipboard = { text, cells, values, source:{...q}, sheetId:this.sheet.id, cut }; this.renderer.copyRange = {...q}; this.renderer.requestFrame(); return text;
  }
  ...
  async paste(mode = 'paste') { let text; try { text = await navigator.clipboard.readText(); } catch { text = this.clipboard?.text; } if (text === undefined) { this.toast('Use Ctrl/⌘ V to paste from your clipboard.'); this.host.focus(); return; } this.pasteText(text,mode); }
  pasteText(text, mode = 'paste') {
    if (!this.editable()) return;
    const sheet = this.sheet, internal = this.clipboard?.text === text ? this.clipboard : null, values = internal ? internal.cells : parseDelimited(text,'\t');
    const h = values.length, w = Math.max(...values.map(row=>row.length)), q = this.selection;
    const block = q.r2-q.r1+1 < MAX_ROWS && q.c2-q.c1+1 < MAX_COLS && (q.r1!==q.r2 || q.c1!==q.c2);
    const r0 = block ? q.r1 : this.active.r, c0 = block ? q.c1 : this.active.c;
    const height = block ? Math.max(h,q.r2-q.r1+1) : h, width = block ? Math.max(w,q.c2-q.c1+1) : w;
    if (height%h || width%w) throw new Error('Select one destination cell, or a range whose dimensions are multiples of the copied block.');
    if (height*width>MAX_RANGE_CELLS || r0+height>MAX_ROWS || c0+width>MAX_COLS) throw new Error('The pasted range exceeds sheet or operation limits.');
    const target = {r1:r0,c1:c0,r2:r0+height-1,c2:c0+width-1};
    if (sheet.merges.some(m=>m.r1<=target.r2 && m.r2>=target.r1 && m.c1<=target.c2 && m.c2>=target.c1)) throw new Error('Unmerge destination cells before pasting.');
    const apply = () => this.errorBoundary(() => {
      if (sheet.protected) throw new Error('The destination sheet is read-only.');
      this.workbook.transaction(internal?.cut ? 'Move cells' : 'Paste cells',()=>{
        ...
        for (let ri=0;ri<height;ri++) for (let ci=0;ci<width;ci++) {
          const sr=ri%h, sc=ci%w, cell=internal ? structuredClone(values[sr][sc] || {raw:''}) : {raw:String(values[sr][sc] ?? '')};
          if (mode==='paste-values') { const value=internal ? internal.values[sr][sc] : cell.raw; cell.raw=value==null ? '' : typeof value==='string' ? "'"+value : String(value); }
          else if (internal && !internal.cut) cell.raw=shiftFormula(cell.raw,r0+ri-sr-internal.source.r1,c0+ci-sc-internal.source.c1);
          if (internal && mode==='paste') { this.workbook.setCell(sheet,r0+ri,c0+ci,null); this.workbook.setCell(sheet,r0+ri,c0+ci,cell); }
          else if (internal || mode==='paste-values') this.workbook.setCell(sheet,r0+ri,c0+ci,{raw:cell.raw});
          else this.workbook.setRaw(sheet,r0+ri,c0+ci,cell.raw);
        }
      });
      if (internal?.cut) { this.clipboard=null; this.renderer.copyRange=null; }
      this.select(target,{r:r0,c:c0}); this.host.focus();
    });
    const overwrite=[...sheet.cells].some(([key,cell])=>{const [r,c]=key.split(',').map(Number);return cell.raw && r>=target.r1 && r<=target.r2 && c>=target.c1 && c<=target.c2;});
    if (overwrite) this.confirm('Replace destination data?',`Pasting will replace existing data in ${rangeAddress(target)}.`,apply,'Replace data'); else apply();
  }
  autoSum() {
  ...
  showFile() {
    ...
    $('#reopen-now').onclick = () => this.errorBoundary(() => {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (!saved) { this.toast('No saved workbook is available in this browser.'); return; }
      const workbook = Workbook.fromJSON(JSON.parse(saved)), reopen = () => { this.setWorkbook(workbook); this.closeDialog(); };
      if (this.startupPending) reopen(); else this.confirm('Reopen last workbook?', 'This replaces the current workbook with its last local autosave. Export your current workbook first to keep a separate copy.', reopen, 'Reopen workbook');
    });
  }
  showExport() {
  ...
  showConditional() {
    if (!this.editable()) return;
    this.openDialog('Conditional formatting', `<p class="help-text">Apply a live rule to <b>${rangeAddress(this.selection)}</b>. Formatting updates when formula results change.</p><div class="dialog-grid"><button class="file-card" data-rule="bars">${icon('chart')}<span><strong>Data bars</strong><small>Compare the magnitude of each value.</small></span></button><button class="file-card" data-rule="scale">${icon('conditional')}<span><strong>Green color scale</strong><small>Shade cells from low to high.</small></span></button><button class="file-card" data-rule="positive">${icon('percent')}<span><strong>Positive / negative</strong><small>Green gains and red losses.</small></span></button><button class="file-card" data-rule="greater">${icon('check')}<span><strong>Greater than…</strong><small>Highlight values above a threshold.</small></span></button><button class="file-card" data-rule="contains"><span><strong>Specific Text containing</strong><small>Highlight cells containing the text below.</small></span></button><button class="file-card" data-rule="not-contains"><span><strong>Specific Text not containing</strong><small>Highlight nonblank cells without the text below.</small></span></button><button class="file-card" data-rule="duplicates"><span><strong>Duplicate values</strong><small>Highlight repeated values in the selection. Ignores case and blanks.</small></span></button><button class="file-card" data-rule="blank"><span><strong>Blank</strong><small>Highlight empty cells, including formulas returning empty text.</small></span></button></div><div id="cf-text-options" hidden><label class="field-label" for="cf-text">Specific text (case-insensitive)</label><input id="cf-text" class="dialog-input" type="text" required></div><div id="cf-threshold-options" hidden><label class="field-label" for="cf-threshold">Threshold for “Greater than”</label><input id="cf-threshold" class="dialog-input" type="number" value="0" required></div><div class="dialog-actions"><button id="clear-rules" class="secondary-btn">Clear sheet rules</button><button class="secondary-btn" data-action="close-dialog">Cancel</button><button id="cf-apply" class="primary-btn" disabled>Apply</button></div>`);
    let type;
    $$('[data-rule]').forEach(button => button.onclick = () => {
      type = button.dataset.rule;
      $$('[data-rule]').forEach(choice => { choice.classList.toggle('active', choice === button); choice.setAttribute('aria-pressed', String(choice === button)); });
      $('#cf-text-options').hidden = type !== 'contains' && type !== 'not-contains';
      $('#cf-threshold-options').hidden = type !== 'greater';
      $('#cf-apply').disabled = false;
    });
    $('#cf-apply').onclick = () => this.errorBoundary(() => {
      if (!type) return;
      const textRule = type === 'contains' || type === 'not-contains';
      if (textRule && !$('#cf-text').reportValidity() || type === 'greater' && !$('#cf-threshold').reportValidity()) return;
      const rule = {type,range:{...this.selection},value:textRule ? $('#cf-text').value : Number($('#cf-threshold').value)};
      this.workbook.mutate('Conditional formatting',()=>this.sheet.conditionalRules.push(rule)); this.closeDialog(); this.toast('Conditional formatting applied. Matching cells are highlighted.');
    });
  ...
  showAnalyzeData() {
    const sheet = this.sheet, q = {...this.selection};
    if (q.c1 !== q.c2) throw new Error('Select one column to analyze.');
    const whole = q.r1 === q.r2 || q.r1 === 0 && q.r2 === MAX_ROWS-1;
    if (whole) { q.r1=0; q.r2=MAX_ROWS-1; }
    this.openPanel('analyze','Analyze Data',`<p class="help-text">${whole ? 'Column '+colName(q.c1) : rangeAddress(q)} · Counts matching displayed values, including filtered rows. Case and spaces are significant.</p><p><label><input id="analyze-header" type="checkbox"${q.r1===0 && sheet.raw(0,q.c1) ? ' checked' : ''}> First row is a header</label></p><button id="analyze-refresh" class="secondary-btn">Refresh</button><p id="analyze-summary" class="help-text"></p><div id="analyze-results"></div>`);
    const render = () => {
      const first=q.r1+($('#analyze-header').checked?1:0), end=whole ? sheet.populatedRange(q).r2 : q.r2, last=whole && !sheet.raw(end,q.c1) ? -1 : end, counts=new Map(); let filled=0;
      for (const [key] of sheet.cells) {
        const [r,c]=key.split(',').map(Number); if(c!==q.c1 || r<first || r>last) continue;
        const value=this.workbook.display(sheet,r,c); if(value==='') continue;
        counts.set(value,(counts.get(value)||0)+1); filled++;
      }
      const total=Math.max(0,last-first+1), blank=total-filled, rows=[...counts].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0]));
      $('#analyze-summary').textContent=`${total.toLocaleString()} rows · ${counts.size.toLocaleString()} distinct values · ${blank.toLocaleString()} blank${counts.size>200 ? ' · Showing the 200 most frequent values' : ''}`;
      const label=$('#analyze-header').checked ? this.workbook.display(sheet,q.r1,q.c1) : 'Value';
      $('#analyze-results').innerHTML=total ? `<table class="help-table" style="table-layout:fixed"><thead><tr><th style="text-align:left;overflow-wrap:anywhere">${escapeHTML(label || 'Value')}</th><th style="width:75px;text-align:right">Count</th></tr></thead><tbody>${rows.slice(0,200).map(([value,count])=>`<tr><td style="overflow-wrap:anywhere;white-space:pre-wrap">${escapeHTML(value)}</td><td>${count.toLocaleString()}</td></tr>`).join('')}${blank ? `<tr><td><em>(blank)</em></td><td>${blank.toLocaleString()}</td></tr>` : ''}</tbody></table>` : '<p class="empty-panel">No data to analyze.</p>';
    };
    $('#analyze-header').onchange=render; $('#analyze-refresh').onclick=()=>this.errorBoundary(render); render();
  }
  showFind() {
