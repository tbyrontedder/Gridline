...
class GridlineApp {
  ...
  showFind() {
    ...
    $('#replace-all').onclick=()=>this.errorBoundary(()=>{if(!this.editable())return;const query=$('#find-query').value,replace=$('#replace-query').value;if(!query)return;let count=0;this.workbook.transaction('Replace all',()=>{for(const [key,cell]of this.sheet.cells){const [r,c]=key.split(',').map(Number);if(!includes(r,c))continue;const displayed=$('#find-look-in').value==='display',text=displayed?this.workbook.display(this.sheet,r,c):cell.raw;if(!text.includes(query))continue;const changed=text.split(query).join(replace);this.workbook.setRaw(this.sheet,r,c,displayed&&changed ? "'"+changed : changed);count++;}});this.toast(`Replaced exact, case-sensitive matches in ${count} cells.`);search();});$('#find-query').focus();
  }
  showNotes() {
